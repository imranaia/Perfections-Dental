...
records_bp = Blueprint('doctor_records', __name__,
                       url_prefix='/api/doctor/records')
...
@records_bp.route('/clinic-info', methods=['GET'])
@login_required
@doctor_required
def get_clinic_info():
    ...
    try:
        ...
        with db.cursor() as cursor:
            ...
            cursor.execute("""
                ...
                LIMIT 10
            """, (doctor_id, search_term, search_term, search_term, search_term))
...
@records_bp.route('/visit/<int:appointment_id>', methods=['GET'])
@login_required
@doctor_required
def get_visit_details(appointment_id):
    ...
    try:
        ...
        with db.cursor() as cursor:
            ...
            # Get teeth findings from notes content (parse structured data if present)
...
@records_bp.route('/visit/<int:note_id>/update', methods=['PUT'])
@login_required
@doctor_required
def update_medical_notes(note_id):
    """Update medical notes content"""
    try:
        doctor_id = session.get('user_id')
        ...
        with db.cursor() as cursor:
            # Check if note exists and belongs to this doctor
            cursor.execute("""
                SELECT mn.id, mn.patient_id, mn.author_id
            ...
            cursor.execute("""
                ...
                VALUES (?, 'UPDATE_MEDICAL_NOTE', 'medical_notes', ?, ?)
            """, (doctor_id, note_id, json.dumps({'content_length': len(new_content)})))
        ...
        print(f"Update medical notes error: {e}")
